...
export default function SearchPage() {
    ...
    const fetchListings = async () => {
      ...
      try {
       ...
       if (queryParam && !categoryParam) {
          // 🚀 THE FIX: Use .includes() so "electronics" matches "Electronics & Home Appliances"
          const matchedMainCat = CATEGORIES_DATA.find(c => 
        ...
        // 🚀 GEOSPATIAL QUERY: Sending lat/lng directly to the backend
  ...
  // 🧠 GPS TEXT-STEALER: Uses the backend-calculated distance from MongoDB
  let autoLocationText = "";
  if (latParam && lngParam && !locationParam && realListings.length > 0) {
    // MongoDB's $geoNear returns results sorted by distance. 
    // We grab the address from the very first (closest) item.
    const nearestAd = realListings[0];
    if (nearestAd && nearestAd.distance < 50) {
  ...
  const filteredListings = realListings.filter((item: any) => {
    ...
    if (queryParam && !categoryParam) {
      // 🚀 THE FIX: Apply the same flexible matching to the frontend filter
      const matchedMainCat = CATEGORIES_DATA.find(c => 
        c.name.toLowerCase().includes(queryParam.toLowerCase())
      );
  ...
  const sortedListings = [...filteredListings].sort((a, b) => {
    ...
    // 🚀 GEOSPATIAL SORTING: Uses the 'distance' field returned by MongoDB $geoNear
  ...
  const getCategoryUrl = (catName: string) => {
    ...
    // 🚀 THE FIX: Stop using 'locationParam' from the old URL. Read fresh memory instead!
            ...
            <div className="md:hidden relative z-30">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (googleObj && googleObj.maps && googleObj.maps.places) {
                          ...
                          service.getPlacePredictions({ input: val, componentRestrictions: { country: "pk" } }, (predictions: any, status: any) => {
              ...
              {activeFilterModal === 'category' && (
                <div className="absolute top-full left-0 w-[90vw] max-w-[300px] bg-white border border-gray-200 rounded-lg shadow-xl mt-1 max-h-[60vh] overflow-y-auto z-50">
                  <Link href="/search" onClick={() => setActiveFilterModal(null)} className="block p-4 text-sm font-bold text-blue-600 border-b border-gray-100 bg-gray-50">All Categories</Link>
                    ...
                    return (
                      <div key={mainCat.name} className="border-b border-gray-100 last:border-0">
                        <div className="flex justify-between items-center p-4 hover:bg-gray-50">
                          <Link href={`/search?category=${encodeURIComponent(mainCat.name)}`} onClick={() => setActiveFilterModal(null)} className="font-bold text-gray-800 text-sm flex-grow">{mainCat.name}</Link>
                          <button onClick={() => toggleSidebarCat(mainCat.name)} className="p-1"><FiChevronDown className={`text-gray-500 transition-transform ${isExpanded ? 'rotate-180' : ''}`} /></button>
                        </div>
                        {isExpanded && (
                          <div className="bg-gray-50 pl-4 py-2 border-t border-gray-100">
                            {mainCat.subcategories.map(subCat => (
                              <Link key={subCat} href={`/search?category=${encodeURIComponent(subCat)}`} onClick={() => setActiveFilterModal(null)} className="block py-2 px-4 text-sm text-gray-600 hover:text-blue-600">{subCat}</Link>
                            ))}
                          </div>
                        )}
                      </div>
          ...
          <div className="flex flex-col md:flex-row gap-6">
            <aside className="hidden md:block w-full md:w-1/4 space-y-4">
              <div className="bg-white p-4 rounded border border-gray-200">
                ...
                <ul className="text-sm space-y-1">
                  <li className="mb-3">
                    <Link href={getCategoryUrl("")} className={!categoryParam ? "font-bold text-blue-600 block" : "text-gray-800 font-bold hover:text-blue-500 block"}>All Categories</Link>
                  </li>
                  {CATEGORIES_DATA.map((mainCat) => {
                    ...
                    return (
                      <li key={mainCat.name} className="mt-1">
                        <div className="flex justify-between items-center py-1.5 rounded-md transition-colors hover:bg-gray-50">
                          <Link href={getCategoryUrl(mainCat.name)} className={`block flex-grow font-semibold ${isMainActive ? "text-blue-600" : "text-gray-700 hover:text-blue-500"}`}>{mainCat.name}</Link>
                          <button onClick={() => toggleSidebarCat(mainCat.name)} className="p-1 text-gray-400 hover:text-blue-600 rounded-full"><FiChevronDown className={`transition-transform duration-200 ${isExpanded ? "rotate-180" : ""}`} size={16} /></button>
                        </div>
                        <div className={`grid transition-all duration-300 ease-in-out ${isExpanded ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
                          <ul className="overflow-hidden pl-4 mt-1 border-l-2 border-blue-100 ml-2 space-y-1">
                            {mainCat.subcategories.map(subCat => (
                              <li key={subCat}><Link href={getCategoryUrl(subCat)} className={`block text-[13px] py-1 px-2 rounded-md hover:bg-blue-50/50 ${categoryParam === subCat ? "text-blue-600 font-bold" : "text-gray-500 hover:text-blue-600"}`}>{subCat} <span className="text-gray-400">({getSubcategoryCount(subCat)})</span></Link></li>
                            ))}
                          </ul>
                        </div>
                      </li>
            ...
            <main className="w-full md:w-3/4">
              <div className="flex justify-between items-center mb-4 pb-4 border-b border-gray-200">
                <span className="text-sm text-gray-500">{isLoading ? "Loading..." : `${sortedListings.length} ads found`}</span>
                <div className="relative">
                  ...
                  {isSortOpen && (
                    <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-gray-200 rounded-md shadow-lg z-20 py-1">
                      {sortOptions.map((option) => (
                        <div key={option} onClick={() => { setSelectedSort(option); setIsSortOpen(false); }} className="flex items-center justify-between px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 cursor-pointer">
                          <span>{option}</span>
