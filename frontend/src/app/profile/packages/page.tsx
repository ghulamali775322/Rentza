...
export default function PackagesPage() {
  ...
  const handleCheckout = () => {
    alert(
      `Proceeding to checkout with ${selectedPlan.name} Plan (PKR ${selectedPlan.price})`,
    );
  };

  return (
    <ProtectedRoute>
      // Container: max-w-1000px, margin auto, px-5 py-10, pt-40px (approx 10 in
      tailwind), min-h-screen, bg-gray
      <div className="max-w-[1000px] mx-auto px-5 py-10 pt-[40px] font-['Helvetica_Neue',_Arial,_sans-serif] min-h-screen bg-[#f8f9fa]">
        ...
        <div className="text-center mb-[50px]">
          {/* Title */}
          <h1 className="text-[32px] font-semibold text-[#002f34] mb-2.5">
            Upgrade your plan
          </h1>
          {/* Subtitle */}
          <p className="text-base text-[#666]">
            Sell faster with our premium exposure packages
          </p>
        </div>
          ...
          {PACKAGES.map((plan) => {
            ...
            return (
              <div
                ...
                style={{
                  ...
                  boxShadow: isSelected
                    ? `0 10px 30px -10px ${plan.color}66`
              ...
              >
                ...
                {/* PlanName */}
                <h3 className="text-2xl font-semibold text-[#002f34] mb-2.5 text-left">
                  {plan.name}
                </h3>
                ...
                >
                  PKR {plan.price}{" "}
                  <span className="text-base text-[#888] font-medium">
                    / {plan.duration}
                  </span>
                </div>
                ...
                <ul className="list-none p-0 m-0 mb-[30px]">
                  {plan.features.map((feature, idx) => (
                    ...
                    <li
                      key={idx}
                      className="flex items-center gap-2.5 text-[15px] text-[#555] mb-3"
                    >
                      <FiCheck
                        size={18}
                        className="text-[#008000] flex-shrink-0"
                      />{" "}
                      {feature}
                    </li>
                  ))}
        ...
        <div className="bg-white border border-[#ddd] rounded-lg p-[30px] flex justify-between items-center shadow-[0_-5px_20px_rgba(0,0,0,0.05)] max-md:flex-col max-md:gap-5 max-md:text-center">
          {/* SummaryText */}
          <div>
            <h3 className="text-xl font-bold text-[#002f34] mb-[5px]">
              Selected: {selectedPlan.name} Package
            </h3>
            <p className="text-[#666]">
              Total to pay: <strong>PKR {selectedPlan.price}</strong>
            </p>
          </div>
