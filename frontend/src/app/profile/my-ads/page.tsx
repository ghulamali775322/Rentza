  ...
  useEffect(() => {
    const fetchMyAds = async () => {
      ...
      try {
        ...
        if (mongoUserId) {
          const response = await fetch(`http://localhost:5000/api/listings/lender/${mongoUserId}`);
        ...
        // 3. If Google Login (no Mongo ID on frontend), fallback to fetching all and filtering by name
          ...
          const result = await response.json();
          if (result.success) {
           const googleUserAds = result.data.filter((item: any) => item.lenderId?.name === session?.user?.name);
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            <div className="text-center py-20 bg-white rounded border border-gray-200">
              ...
              <p className="text-gray-500 mt-2">You haven't posted any ads yet, or none match your search.</p>
            </div>
        )}
      </div>
    </div>
