 ...
 // 1. The Search Logic
      ...
      if (localToken) {
        try {
          ...
          if (data.profilePhotoPath) {
            setFetchedPhoto(
              `http://localhost:5000${data.profilePhotoPath}?t=${Date.now()}`,
            );
          }
        } catch (e) {
  ...
  const fetchNotificationsOnly = useCallback(async () => {
    ...
    try {
      ...
      if (localToken) authHeaders["Authorization"] = `Bearer ${localToken}`;
        ...
        authHeaders["x-google-email"] = session.user.email;

      const notifRes = await fetch(
        `http://localhost:5000/api/notifications?t=${Date.now()}`,
        { headers: authHeaders },
      );
      const notifResult = await notifRes.json();

      if (notifResult.success) {
        const sorted = notifResult.data.sort((a: any, b: any) =>
    ...
    const fetchBadgeData = async () => {
      ...
      try {
        const planRes = await fetch(
          `http://localhost:5000/api/subscriptions/status/${id}`,
        );
      ...
      try {
        const res = await fetch(
          `http://localhost:5000/api/chat/unread-count/${id}?t=${Date.now()}`,
        );
  ...
  useEffect(() => {
    ...
    const refreshBadge = async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/api/chat/unread-count/${myMongoId}?t=${Date.now()}`,
        );
        const result = await res.json();
        if (result.success) setUnreadCount(result.count || 0);
      } catch (err) {}
    ...
    <nav className="w-full fixed top-0 left-0 z-50">
      ...
      >
        ...
        <div className="flex items-center gap-3 md:gap-4 relative">
          <Link
            href="/about"
            className="md:hidden text-[13px] font-bold text-gray-600 hover:text-[#0077ff]"
          >
            About Us
          </Link>
          {!hasMounted ? (
            ...
            <div className="hidden md:flex items-center gap-4">
              ...
              <div ref={notifRef} className="relative">
                <div
                  ...
                  className="icon-btn flex items-center justify-center cursor-pointer relative"
                >
                      ...
                      {notifications.length === 0 ? (
                        ...
                        notifications.slice(0, 5).map((n) => (
                          <div
                            key={n._id}
                            onClick={() =>
                              handleReadNotification(n._id, n.isRead)
                            }
              ...
              <div ref={profileRef} className="relative">
                ...
                >
                  <div
                    className={`rounded-full ${userPlan === "premium" ? "p-[2.5px] bg-gradient-to-tr from-indigo-500 via-purple-500 to-pink-500 shadow-sm" : userPlan === "gold" ? "p-[2.5px] bg-gradient-to-tr from-yellow-500 via-yellow-200 to-yellow-600 shadow-sm" : ""}`}
                  >
                    <div
                      className={`w-9 h-9 rounded-full overflow-hidden bg-[#0077ff] flex items-center justify-center text-white font-bold ${userPlan !== "free" ? "border-2 border-white" : ""}`}
                    >
                      {/* FIXED LOGIC HERE: prioritizing Context over Fetched */}
                      {profilePhotoUrl || fetchedPhoto ? (
                        <img
                          src={profilePhotoUrl || fetchedPhoto || ""}
                          alt="Profile"
                ...
                {showProfileMenu && (
                  <div className="absolute top-12 right-0 w-64 bg-white rounded-lg shadow-lg border border-gray-200 p-3 z-50 animate-fadeIn">
                    <div className="flex items-center gap-3 border-b border-gray-100 pb-2 mb-2">
                      ...
                      >
                        <div
                          className={`rounded-full ${userPlan === "premium" ? "p-[3px] bg-gradient-to-tr from-indigo-500 via-purple-500 to-pink-500 shadow-md" : userPlan === "gold" ? "p-[3px] bg-gradient-to-tr from-yellow-500 via-yellow-200 to-yellow-600 shadow-md" : ""}`}
                        >
                          <div
                            className={`w-10 h-10 rounded-full flex items-center justify-center bg-[#0077ff] text-white font-bold ${userPlan !== "free" ? "border-2 border-white" : ""}`}
                          >
                            {/* FIXED LOGIC HERE: prioritizing Context over Fetched */}
                            {profilePhotoUrl || fetchedPhoto ? (
                              <img
          ...
          >
            <div className="bg-white rounded-full px-3 sm:px-6 py-2 flex items-center gap-1 sm:gap-2 transition-all duration-300 hover:bg-[#f9fafb]">
              ...
              <span className="hidden sm:inline font-bold text-[#002f34] tracking-wide">
                POST AD
              </span>
            </div>
          </button>
        ...
        <div className="max-w-6xl mx-auto min-h-[64px] py-2 md:py-0 flex flex-col md:flex-row md:items-center gap-2 md:gap-3 px-4 z-[60]">
          ...
          <div className="flex items-center border rounded-lg bg-white h-10 w-full md:flex-1 overflow-hidden transition hover:border-[#0077ff] order-1 md:order-2">
            ...
            <button
              ...
              className="bg-[#002f34] text-white flex items-center justify-center gap-1 sm:gap-2 px-3 sm:px-4 h-full rounded-r-lg hover:bg-[#004247] cursor-pointer"
            >
      ...
      <div
        className={`${isListingPage ? "hidden" : "md:hidden"} fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 shadow-[0_-4px_10px_rgba(0,0,0,0.05)] z-[100] pb-2 pt-2 px-6`}
      >
        <div className="flex justify-between items-center h-14">
          {/* 1. Home */}
          <Link
            href="/"
            className={`flex flex-col items-center gap-1 transition-colors ${pathname === "/" ? "text-[#0077ff]" : "text-gray-500 hover:text-gray-900"}`}
          >
            <FiHome size={22} />
            <span
              className={`text-[10px] ${pathname === "/" ? "font-bold" : "font-medium"}`}
            >
              Home
            </span>
          </Link>
          ...
          <Link
            ...
            className={`flex flex-col items-center gap-1 relative transition-colors ${pathname.includes("/inbox") ? "text-[#0077ff]" : "text-gray-500 hover:text-gray-900"}`}
          >
            <FiMessageCircle size={22} />
            {unreadCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-[#f62d51] text-white text-[8px] font-bold h-3 w-3 flex items-center justify-center rounded-full border border-white"></span>
            )}
            <span
              className={`text-[10px] ${pathname.includes("/inbox") ? "font-bold" : "font-medium"}`}
            >
              Chat
            </span>
          </Link>
          ...
          <div className="flex flex-col items-center relative">
            ...
            <span className="text-[10px] font-bold text-[#002f34] mt-7">
              Post Ad
            </span>
          </div>
          ...
          {/* 4. Notifications */}
          <Link
            href={isLoggedIn ? "/profile/notifications" : "/login"}
            className={`flex flex-col items-center gap-1 relative transition-colors ${pathname.includes("/notifications") ? "text-[#0077ff]" : "text-gray-500 hover:text-gray-900"}`}
          >
            <FiBell size={22} />
            {unreadNotifCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-[#f62d51] text-white text-[8px] font-bold h-3 w-3 flex items-center justify-center rounded-full border border-white"></span>
            ...
            <span
              className={`text-[10px] ${pathname.includes("/notifications") ? "font-bold" : "font-medium"}`}
            >
              Alerts
            </span>
          </Link>
          ...
          >
            ...
            >
              <FiUser size={22} />
              <span
                className={`text-[10px] ${pathname.includes("/profile") || showMobileProfileMenu ? "font-bold" : "font-medium"}`}
              >
                Profile
              </span>
            </button>
              ...
              <div className="absolute bottom-16 right-2 w-64 bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden z-[110] animate-fadeIn">
                ...
                <div className="flex items-center gap-4 p-4 border-b border-gray-100 bg-gray-50/50">
                  ...
                  <Link
                    ...
                    className="relative shrink-0 block cursor-pointer transition hover:opacity-80"
                  >
                    ...
                    >
                      <div
                        className={`relative w-12 h-12 rounded-full flex items-center justify-center bg-[#0077ff] text-white font-bold overflow-visible ${userPlan !== "free" ? "border-2 border-white" : ""}`}
                      >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <Link
                      href={
                        myMongoId ? `/lender/${myMongoId}` : "/profile/my-ads"
                      }
                      onClick={() => setShowMobileProfileMenu(false)}
                ...
                <div className="flex flex-col text-sm text-gray-700 font-medium">
                  <Link
                    href="/profile/packages"
                    onClick={() => setShowMobileProfileMenu(false)}
                    className="px-4 py-3 hover:bg-gray-50 border-b border-gray-50 flex items-center gap-3 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="px-4 py-3 text-red-500 hover:bg-red-50 flex items-center gap-3 text-left w-full transition-colors"
                  >
                    <FiLogOut size={18} /> Logout
                  </button>
                </div>
